...
export default function PlayerCard({ character, showStats = true }: PlayerCardProps) {
  ...
  return (
    <div className="bg-[#101012] border border-[#27272A] rounded-lg p-4 hover:border-[#3F3F46] transition-colors cursor-pointer group">
      ...
      {showStats !== false ? (
        ...
        <div className="h-24 bg-[#0D0D0F] rounded-lg border border-[#1C1C1E] flex items-center justify-center">
          <span className="text-[#3F3F46] text-xs italic tracking-wide">Status ocultado pelo Mestre</span>
        </div>
      )}
    </div>
  )

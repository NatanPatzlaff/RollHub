...
export default function RollHistorySidebar({ rolls = [] }: RollHistorySidebarProps) {
  return (
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        {rolls.length === 0 && (
          <div className="text-center py-20 px-6 opacity-20">
            ...
            <p className="text-sm">Nenhuma rolagem recente.</p>
          </div>
        )}
        
        {rolls.map(roll => {
          let borderColor = 'border-[#27272A]'
          let icon = <CheckCircle2 size={14} className="text-[#10B981]" />
            ...
            <div key={roll.id} className={`bg-[#101012] border ${borderColor} rounded-lg p-3 text-sm relative`}>
              <div className="flex justify-between items-center mb-1">
                <span className={`font-bold ${roll.isGM ? 'text-[#7C3AED]' : 'text-[#A1A1AA]'}`}>
                  {roll.userName || roll.playerName}
                </span>
                <span className="text-[10px] text-[#52525B]">
                  {roll.createdAt ? new Date(roll.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                </span>
              </div>
              <div className="text-[#D4D4D8] mb-2">{roll.title || roll.action}</div>
              
              <div className="flex justify-between items-center bg-[#09090B] p-2 rounded border border-[#27272A]">
                <span className="text-xs text-[#8B8B94] font-mono">{roll.expression || roll.roll}</span>
                <div className="flex items-center gap-1.5">
                  {icon}
                  <span className={`text-xl ${textColor}`}>{roll.total || roll.result}</span>
                </div>
              </div>
            </div>
          )

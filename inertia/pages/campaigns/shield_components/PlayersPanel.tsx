...
export default function PlayersPanel({ entities }: PlayersPanelProps) {
    ...
    <div className="bg-[#18181B] border-r border-[#27272A] flex flex-col h-full overflow-hidden w-64 xl:w-72 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        {sortedEntities.map((entity, index) => {
          ...
          const pePercent = entity.maxPe > 0 ? (entity.pe / entity.maxPe) * 100 : 0
          const sanPercent = entity.maxSanity > 0 ? (entity.sanity / entity.maxSanity) * 100 : 0
          const isTurn = index === 0

          return (
            <div key={entity.id} className={`bg-[#101012] border ${isTurn ? 'border-[#F97316]' : entity.isMonster ? 'border-[#EF4444]/50' : 'border-[#27272A]'} rounded-lg p-3 hover:border-[#3F3F46] transition-colors relative`}>
              ...
              <div className="flex justify-between items-start mb-2 pr-8">
                <h3 className={`font-bold text-sm truncate ${entity.isMonster ? 'text-[#EF4444]' : 'text-white'}`}>{entity.name}</h3>
              </div>
              <span className="block text-[10px] font-bold uppercase tracking-wider text-[#A1A1AA] mb-2">
                {entity.isMonster ? 'Criatura' : entity.class}
              </span>
              ...
              <div className="space-y-2">
                {!entity.isMonster && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white font-mono w-8 text-right">{entity.hp}</span>
                  </div>
                )}
                {!entity.isMonster && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white font-mono w-8 text-right">{entity.pe}</span>
                  </div>
                )}
                {!entity.isMonster && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white font-mono w-8 text-right">{entity.sanity}</span>
                  </div>
                )}
              </div>
            </div>
